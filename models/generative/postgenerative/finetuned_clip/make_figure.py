...
# Bin the Scores into intervals of 0.05
bins = np.arange(0, 1.05, 0.05)  # Create bins from 0 to 1 with step size 0.05
data['Binned Scores'] = pd.cut(data['Scores'], bins=bins, labels=np.round(bins[:-1], 2))

print(data.head())

# Create a Plotly boxplot treating Predictions as a continuous variable and using Binned Scores
fig = px.box(
    data, 
    x='Binned Scores', 
    y='Predictions',
    title='Boxplot of CLIP Finetuned predictions and the Binned P@10 scores for the CLIP retrieval model',
    labels={'Predictions': 'Predicted Score', 'Binned Scores': 'Ground Truth Scores (Binned)'},
    width=600, 
    height=400
)
...
# Save as a PDF file
fig.write_image('./glide_clip_finetuned_boxplot.pdf', format='pdf')
'''