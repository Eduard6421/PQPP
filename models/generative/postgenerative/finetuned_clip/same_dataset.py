...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, gt_path, accepted_source):
        ...
        for i in range(len(loaded_dataset)):
            if i // 4 in indices:
                self.dataset.append(loaded_dataset[i])
...
train_loader = torch.utils.data.DataLoader(
    CustomDataset(train_dataset, train_data_path, train_source), batch_size=256, shuffle=True
)

validation_loader = torch.utils.data.DataLoader(
    CustomDataset(validation_dataset, val_data_path,train_source), batch_size=256, shuffle=True
)

test_loader = torch.utils.data.DataLoader(
    CustomDataset(test_dataset,test_data_path, test_source), batch_size=32, shuffle=False
)
    ...
    for decay in hyperparameters["weight_decay"]:
        ...
        for epoch in range(num_epochs):
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(model.state_dict(), fr"./same_ds_crossmodel_{train_source}_{selected_model}_best_model.pt")
...
model = NeuralNetworkRegressor().to(device)
model.load_state_dict(torch.load(fr"./same_ds_crossmodel_{train_source}_{selected_model}_best_model.pt"))
model.eval()
...
pickle.dump(test_predictions, open(fr"./same_ds_crossmodel_{train_source}_{selected_model}_test_predictions.pickle", "wb"))
