...
class CustomDataset(Dataset):
    def __init__(self, pickle_file, score_file_path):
        with open(pickle_file, "rb") as f:
            self.data = pickle.load(f)
            self.scores = pd.read_csv(score_file_path)['score'].tolist()
...
# DataLoader setup
train_dataset = CustomDataset(fr"correlation_cnn_train_data.pkl", train_data_path)
validation_dataset = CustomDataset(fr"correlation_cnn_val_data.pkl", val_data_path)
test_dataset = CustomDataset(fr"correlation_cnn_test_data.pkl",test_data_path)
        ...
        for wd in hyperparameters["weight_decay"]:
            ...
            for epoch in range(epochs):
                ...
                if val_loss < best_val_loss:
                    ...
                    torch.save(model.state_dict(),fr"{selected_model}_best_model.pth")
                    print(
...
model.load_state_dict(torch.load(fr"{selected_model}_best_model.pth"))
model.eval()
predictions = []
with torch.no_grad():
...
with open(fr"{selected_model}_test_predictions.pickle", "wb") as f:
