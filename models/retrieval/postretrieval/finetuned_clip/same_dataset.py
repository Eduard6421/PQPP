...
validation_dataset = pickle.load(open("./val_dataset.pickle", "rb"))
test_dataset = pickle.load(open("./test_dataset.pickle", "rb"))


# this is used only to get the indexes of the mscoco / drawbench
selected_model = "clip"
train_source = "mscoco"
test_source = "mscoco"
...
train_loader = torch.utils.data.DataLoader(
    CustomDataset(train_dataset, train_retrieval_gt_path, train_source), batch_size=256, shuffle=True
)

validation_loader = torch.utils.data.DataLoader(
    CustomDataset(validation_dataset, val_retrieval_gt_path, train_source), batch_size=256, shuffle=True
)

test_loader = torch.utils.data.DataLoader(
    CustomDataset(test_dataset, test_retrieval_gt_path, test_source), batch_size=256, shuffle=False
)
    ...
    for decay in hyperparameters["weight_decay"]:
        ...
        for epoch in range(num_epochs):
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(model.state_dict(), fr"./same_dataset_{train_source}.pt")
...
model = NeuralNetworkClassifier().to(device)
model.load_state_dict(torch.load(fr"./same_dataset_{train_source}.pt"))
model.eval()
...
pickle.dump(test_predictions, open(fr"./same_dataset_unprocessed_{train_source}_predictions.pickle", "wb"))
