...
class CustomDataset(Dataset):
    ...
    def __init__(self, pickle_file, scores_array, gt_file_path, accepted_source ):

        gt_data = pd.read_csv(gt_file_path)

        # get the indexes of scores that have source in accepted_sources
        indices = gt_data[gt_data['source'] == accepted_source].index.tolist()

        data = pickle.load(open(pickle_file, "rb"))

        self.data = [data[i] for i in indices]
        self.scores = scores_array[indices]
...
# DataLoader setup
train_dataset = CustomDataset("corr_train_dataset.pickle", train_array_scores, train_retrieval_gt_path, train_source)
validation_dataset = CustomDataset(
    "corr_val_dataset.pickle", validation_array_scores, val_retrieval_gt_path,train_source
)
test_dataset = CustomDataset("corr_test_dataset.pickle", test_array_scores, test_retrieval_gt_path, test_source)
        ...
        for wd in hyperparameters["weight_decay"]:
            ...
            for epoch in range(epochs):
                ...
                if val_loss < best_val_loss:
                    ...
                    torch.save(model.state_dict(), f"./crossmodel_{train_source}_{selected_model}_{metric}_best_model.pth")
                    print(
...
model.load_state_dict(torch.load(f"./crossmodel_{train_source}_{selected_model}_{metric}_best_model.pth"))
model.eval()
predictions = []
with torch.no_grad():
...
with open(f"./crossmodel_{train_source}_{selected_model}_test_predictions_{metric}.pickle", "wb") as f:
