...
def save_clip_scores(num_queries, model, processor, dataframe):
    result_array = []
    for i in tqdm(range(num_queries), desc="Generating retrieval results"):
        row = dataframe.iloc[i]
        caption = row["best_caption"]
        sorted_image_ids, sorted_scores = retrieve_images(
            model=model, processor=processor, text=caption
        )
        result_array.append(sorted_scores)

    return result_array


retrieval_results = save_clip_scores(10000, model, processor, best_captions_df)
best_captions_df["clip_scores"] = retrieval_results

best_captions_df.to_csv("./clip_retrieval_results.pickle")
